...
");
    ...
    public function trierm($id)
    {
        $date=new \DateTime();
        $d=$date->format('Y-m-d');
        $qb=$this->getEntityManager()->createQuery("select c from FiThnitekBundle:OffreColis c where c.dateCol >='$d'
        and c.idU != $id and c.longueur > '0' and c.largeur > '0'  ORDER BY c.dateCol DESC");
        return $query=$qb->getResult();
    }
    ...
    public function trieprix($id)
    {
        $date=new \DateTime();
        $d=$date->format('Y-m-d');
        $qb=$this->getEntityManager()->createQuery("select c from FiThnitekBundle:OffreColis c where c.dateCol >='$d'
        and c.idU != $id and c.longueur > '0' and c.largeur > '0'  ORDER BY c.prix DESC");
        return $query=$qb->getResult();
    }
